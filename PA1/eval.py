...
try:
    ...
    print("Optional libraries for visualization are not installed. Please install them using: pip install matplotlib pillow")
    flag = False

def compute_embeddings(model, dataset, device, batch_size=32) -> tuple[np.ndarray, list, list]:
    '''
...
def compute_similarity_matrices(q_embs_dict : dict, db_embs_dict : dict, gt : dict) -> tuple[np.ndarray, np.ndarray]:
    '''
    ...
    for query_idx in range(n_queries):
        for db_idx in range(n_db):
            ...
            sim_scores[query_idx, db_idx] = cos_sim.detach().numpy().item() # cosine similarity between query and database embeddings
            ground_truth_dist[query_idx, db_idx] = cord_dist # co-ordinate L2 - norm distance between query and database images
    
    return sim_scores, ground_truth_dist

def evaluate_recall(model, query_ds, db_ds, utmQ, utmDb, posDistThr, device, 
                   recall_values=[1, 5, 10], preview=False, save_dir="output/visualizations", num_examples=5):
    '''
    ...
    sim_scores, ground_truth_dist = compute_similarity_matrices(q_embs_dict, db_embs_dict, 
                                                               {"utmQ": utmQ, "utmDb": utmDb})
    ...
    for query_idx in range(n_queries):
        ...
        top_indices = np.argsort(sim_scores[query_idx])[-max_k:][::-1] # All Similar DB Images sorted by similarity score [MAX K Images]
        
        for k in recall_values: # for each recall value, check if atleast one positive in top k predictions
            top_k_indices = top_indices[:k] # top k predictions for each recall value
            if np.any(np.in1d(top_k_indices, positives)): # atleast one positive in top k predictions 
    ...
    if flag and preview:
        ...
        visualize_retrieval_results(q_embs_dict, db_embs_dict, sim_scores, 
                                  ground_truth_dist, posDistThr, save_dir, 
                                  num_examples=num_examples,
                                  k = 5)
    
    return recalls, sim_scores, ground_truth_dist

def visualize_retrieval_results(q_embs_dict : dict, db_embs_dict : dict, sim_scores : np.ndarray, ground_truth_dist : np.ndarray, 
                              posDistThr : float, save_dir : str, num_examples : int = 5, k : int = 5):
    '''
    ...
    for count, query_idx in enumerate(query_indices):
        ...
        top_k_db_images_ground_truth_dist = [ground_truth_dist[query_idx, idx] for idx in top_k_indices]
        
        # Get ground truth positives
        ground_truth_positive_indices = np.where(ground_truth_dist[query_idx] < ground_truth_threshold)[0]
        ground_truth_positive_images = [db_embs_dict[idx]['path'] for idx in ground_truth_positive_indices]
        ...
        true_positives = np.sum(np.array(top_k_db_images_ground_truth_dist) < ground_truth_threshold)
        ...
        sorted_indices_with_db_idx = list(zip(top_k_indices, top_k_db_images_ground_truth_dist))
        sorted_indices_with_db_idx.sort(key=lambda x: (
        ...
        for i, (db_idx, gt_dist) in enumerate(sorted_indices_with_db_idx):
            ...
            plt.gca().add_patch(plt.Rectangle((0, 0), db_img.width, db_img.height, 
                                             fill=False, edgecolor=border_color, linewidth=4))
            plt.axis("off")
            
            title = (f"Rank {i+1} [{border_label}]\n"
        ...
        if closest_gt_idx is not None:
            ...
            plt.gca().add_patch(plt.Rectangle((0, 0), gt_img.width, gt_img.height, 
            ...
            gt_title = (f"Closest Ground Truth{top_k_status}\n"
